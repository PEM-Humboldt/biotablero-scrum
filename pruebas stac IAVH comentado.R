...
## Cargar coleccion
STACItemCollection_QUEBEC <- STACQuery_QUEBEC %>% rstac::stac_search(collections = STACCollectionList_ids_QUEBEC[1] ) %>% rstac::get_request()
...
STACCollectionList_ids_IAvH # lista de colecciones disponibles en ese server stac


collection_selected<- "Colombia_PP_test" # seleccion de coleccion a usar
...
## Cargar coleccion
STACItemCollection_IAvH <- STACQuery_IAvH %>% rstac::stac_search(collections = collection_selected ) %>% rstac::get_request()
...
## Poligono de prueba
input<- list(wkt_area= "D:/Repositorios/biab-2.0/scripts/00_load_collection/input/wkt_polygon_test.txt",
...
## establecer parametros cube view, ventana temporal y espacial
